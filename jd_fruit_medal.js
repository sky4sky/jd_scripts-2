...
集勋章、赢好礼
活动地址：
https://h5.m.jd.com/babelDiy/Zeus/m6Gntdu86ypN4ehW9oFsChdMtPG/index.html
14 7,21 * * * jd_fruit_medal.js
 */
const $ = new Env('集勋章、赢好礼');
const jdCookieNode = $.isNode() ? require('./jdCookie.js') : '';
const notify = $.isNode() ? require('./sendNotify') : '';
const JD_API_HOST = 'https://api.m.jd.com/client.action';
let cookie = '', cookiesArr = [], allMessage = ''
if ($.isNode()) {
...
async function exchangeAward() {
  const collect_getAwardInfo = await request('collect_getAwardInfo');
  if (collect_getAwardInfo.code === '0' && collect_getAwardInfo.result) {
    const { chooseAwardSubTitle, awardList } = collect_getAwardInfo.result;
    $.log(chooseAwardSubTitle);
    for (const award of awardList) {
      if (award['awardType'] === 3) {
        console.log(`奖品：${award['awardValue']}京豆，${award['risk'] ? '库存不足' : '库存充足'}`)
        if (!award['risk']) {
      ...
      } else if (award['awardType'] === 2) {
        console.log(`奖品：${award['discount']}元无门槛红包，${award['risk'] ? '库存不足' : '库存充足'}`)
        if (!award['risk']) {
      ...
      } else if (award['awardType'] === 1) {
        console.log(`奖品：${award['awardValue']}农场水滴，库存充足`)
        if (!award['risk']) {
...
function request(function_id, body = {}, timeout = 0){
  return new Promise(resolve => {
    setTimeout(() => {
      const options = taskPostUrl(function_id, body);
