...
活动时间：2021-10-20到2021-11-20
默认助力第一个账号，如果第一个账号火爆，则不进行助力
...
let mainPin = '';
let codeList = [];
let autoCode = 'SZm_olqSxIOtH97BATGmKoWraLaw', projectId = '', helpId = '';
!(async () => {
  console.log('活动入口：APP - 京东超市 - 限时抢京豆\n' +
      '活动地址：https://h5.m.jd.com/babelDiy/Zeus/3fCUZv7USx24U1zzhLdFV4oDQ37b/index.html\n' +
      '活动时间：2021-10-20到2021-11-20\n' +
      '默认助力第一个账号，如果第一个账号火爆，则不进行助力')
  if (!cookiesArr[0]) {
  ...
  //======做任务=========
  console.log(`\n\n===========================做任务===========================`)
  let promiseArr = cookiesArr.map((ck, index) => main(ck));
  ...
  //======助力=========
  console.log(`\n\n===========================内部账号助力===========================`)
  promiseArr = cookiesArr.map((ck, index) => help(ck));
  await Promise.all(promiseArr);
  await $.wait(2000);
  ...
  //======领取京豆=========
  console.log(`\n\n===========================领取京豆===========================`)
  promiseArr = cookiesArr.map((ck, index) => main(ck));
  await Promise.all(promiseArr);
})().catch((e) => {
  $.log('', `❌ ${$.name}, 失败! 原因: ${e}!`, '')
}).finally(() => {
...
async function help(ck) {
  try {
    ...
    if (userName === ownCode.user) {
      ...
      console.log(`${userName} 助力 ${ownCode.user}`);
      ...
      if (getInfo.assistFlag === '1') {
        ...
        console.log(`【${userName}】已助力过${ownCode.user}，或者${userName}无助力次数`);
      }
    }
    await $.wait(1000)
  } catch (e) {
